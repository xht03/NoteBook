...
设一个离散时间的线性时不变（LTI）系统，输入为 $x(n)$、输出为 $y(n)$。在信号与系统一章中我们已经论证：**LTI 系统的输出，等于输入信号与系统单位脉冲响应的卷积**：

$$
y(n)=x(n)*h(n)
$$

也就是说，一个 LTI 系统的输出完全由输入信号 $x(n)$ 和系统的单位脉冲响应 $h(n)$ 决定。由于任意输入信号都可以拆分成许多具有不同权重和延迟的单位脉冲，而系统具有线性（可叠加）和时不变性，因此只要知道系统对单位脉冲的响应 $h(n)$，就能得到它对任意输入的响应。
...
其中 $H(e^{j\omega})$ 是系统的**频率响应**，也将 $H(z)$ 称为**系统函数**（见 Z 变换一章）。
