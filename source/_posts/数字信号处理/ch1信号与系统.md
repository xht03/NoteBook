...
线性和时不变性是两个互不相关的概念，系统可以既是线性的又是时不变的，这种系统称为**线性时不变系统**（Linear Time-Invariant，简称 **LTI**）。线性时不变系统具有良好的性质，也是信号处理中主要研究的系统。现实中，严格的时不变系统是几乎不存在的，元件老化必然导致系统参数的变化，但是在一定时间范围内，我们可以近似认为系统是时不变的。
...
对于离散时间信号，只需把积分换成求和，就得到离散卷积的定义：

$$
x(n) * h(n) = \sum_{k=-\infty}^{\infty} x(k) h(n-k)
$$

其中 $x(n)$ 与 $h(n)$ 是两个离散时间信号，$n$ 是卷积的自变量。以后不加说明时，记 $t$ 的信号是连续时间信号，记 $n$ 的信号是离散时间信号。
...
这正是卷积的直观来源：**任何连续时间信号都可以分解成单位冲激函数位移加权后的线性组合**，其权重正是信号本身在各时刻的取值。

![](https://ref.xht03.online/202503310956412.png)

对于离散时间信号同理，任意序列都可以分解为移位单位采样序列的加权叠加：

$$
x(n)=\sum_{k=-\infty}^{+\infty}x(k)\delta(n-k)
$$

所以无论是连续还是离散，卷积之所以成为"自然的运算"，正是因为**信号都可以表示成冲激（单位采样序列）的线性组合**。

---

### 单位冲激响应与 LTI 系统的输出

对于连续时间系统：任何信号都可以分解成单位冲激函数的线性组合，因此只要知道系统对单位冲激函数的响应，就能得到它对任意输入的响应。我们引入系统的**单位冲激响应** $h(t)$，它是系统对单位冲激函数 $\delta(t)$ 的响应。当输入为 $x(t)=\delta(t)$ 时，输出为 $y(t)=h(t)$，因此：

$$
y(t) = \int_{-\infty}^{\infty} x(\tau) h(t-\tau) d\tau = x(t)*h(t)
$$

对于离散时间系统同理。设离散时间 LTI 系统为 $T$，输入为 $x(n)$，输出为 $y(n)=T\{x(n)\}$。定义系统对单位采样序列 $\delta(n)$ 的响应为**单位冲激响应** $h(n)=T\{\delta(n)\}$。若系统具有**时不变性**，则输入延迟 $k$ 个采样点时输出也相应延迟 $k$ 个采样点，即 $h(n-k)=T\{\delta(n-k)\}$。再结合**线性**，将求和与常数系数移出系统算子：
...
总结一下：无论是连续时间还是离散时间，**LTI 系统的输出都等于输入信号与该系统冲激响应的卷积**：

$$
y(t)=x(t)*h(t),\quad y(n)=x(n)*h(n)
$$

这是本节的**核心结论**：一个 LTI 系统对任意输入的响应，完全由它的冲激响应 $h$ 决定。可以说，掌握了系统的冲激响应，就等价于掌握了这个系统的全部行为——这正是后面用卷积、用频率响应分析系统、乃至设计滤波器的根本出发点。
